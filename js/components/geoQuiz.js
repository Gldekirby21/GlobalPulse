...
class GeoQuiz {
    ...
    /** Called on auth state change (sign in, session restore, sign out) */
    setSession(session) {
        if (!this.round) {
            this.renderStart();
        }
        gamificationService.fetchLeaderboard().then((users) => this.renderLeaderboard(users)).catch(console.warn);
    }
